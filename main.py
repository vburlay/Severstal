...
from steps.train import get_resnet50
import os
...
def main():
    ...
    model = get_resnet50()
    model.fit(generator_train, epochs=G.nb_epochs, callbacks=G.callbacks)
    ...
    accuracy, class_report, roc_auc_score = predictor.evaluate_model(X_test,
                                                                     y_test)
...
def train_with_mlflow():
    ...
    with mlflow.start_run() as run:
        ...
        report = classification_report(y_test,
                                       trainer.pipeline.predict(X_test),
                                       output_dict=True)
        logging.info("Model evaluation completed successfully")
